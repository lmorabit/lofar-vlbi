...
def first_download (ra,dec,outfile='first_out.fits',gif=0,fits=1,imsize=2.0,imserver='third.ucllnl.org'):
    ...
    command = ('wget -O %s "https://%s/cgi-bin/firstimage?RA=%s %s %s %s %s %s&Dec=&Equinox=J2000&ImageSize=%.1f&MaxInt=10&GIF=%d&FITS=%d&Download=1 --no-check-certificate"'%(outfile,imserver,s[0],s[1],s[2],s[3],s[4],s[5],imsize,gif,fits))
    os.system(command)
    try: 
