...
def combine_subbands (inarray, nameout, datacol, phasecenter, fres, tres, phscmd, filcmd, nthreads):
    ...
    with open(parset_name,'w') as fo:
        ...
        if phscmd != '000':
            fo.write('steps = [shift,average1,applybeam,adder,filter,average2]\n')
        else:
        ...
        fo.write('average1.tpye = averager\n')
        fo.write('average1.freqresolution = 48.82kHz\n')
        fo.write('average1.timeresolution = 4.\n')
        fo.write('applybeam.type = applybeam\n')
        fo.write('applybeam.beammode = full\n')
	if phscmd != '000':
            fo.write('adder.type = stationadder\n')
            fo.write("adder.stations = %s\n"%phscmd)
            fo.write('filter.type = \'filter\'\n')
            fo.write("filter.baseline = %s\n"%filcmd)
            fo.write('filter.remove = True')
        fo.write('average2.type = averager\n')
        fo.write('average2.freqresolution = 390.56kHz\n')
        fo.write('average2.timeresolution = 32.\n')
    fo.close()
