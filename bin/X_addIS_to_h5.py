...
def main(h5parmfile, MSfiles, cal_solset='calibrator', solset_in='target', solset_out='targetIS'):
	...
	for cal_soltab_name in cal_soltabNames:
		...
		out_axes = ['ant','time']
		out_axes_vals = [soltab.ant, new_times]
		out_lens = (len(soltab.ant), len(new_times))
	    else:
		out_axes = ['ant','time','freq','pol']
	        out_axes_vals = [soltab.ant, new_times, soltab.freq, soltab.pol]
		out_lens = (len(soltab.ant), len(new_times), len(soltab.freq), len(soltab.pol))
	    ## get the values
	    ...
	    print cal_soltab_name

	    ## find median values
	    med_vals = np.nanmedian( vals, axis=1 )
	    med_weights = np.nanmedian( weights, axis=1 )
	    ## interpolate to the new array shape
	    new_vals = np.ndarray( shape=out_lens )
	    new_weights = np.ndarray( shape=out_lens )
	    for x in np.arange(len(new_times)):
	        if len(soltab_axes) == 2:
                    new_vals[:,x] = med_vals
                    new_weights[:,x] = med_weights
		else:
		    new_vals[:,x,:,:] = med_vals
	            new_weights[:,x,:,:] = med_weights
	...
	data.close()
	logging.info('Solutions targetIS already exist, not adding to h5parm.')
	return(0)
...
if __name__ == "__main__":
    ...
    main(h5parmfile, MSfiles, cal_solset=args.cal_solset, solset_in=args.solset_in, solset_out=args.solset_out)
