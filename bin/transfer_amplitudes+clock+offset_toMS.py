...
def main(msname, npdir='', store_basename='', newparmdbext='transfer_amp_clock_sols'):
    ...
    freqs_phase = np.load(npdir + '/freqs_for_phase_array.npy')
    phases_array  = np.load(npdir + '/' + store_basename + '_phase_array.npy')
    ...
    for antenna in msinfo.stations:
        try:
            antenna_id = np.where(station_names == antenna)[0][0]
            print 'Filling up ',antenna,antenna_id
            ...
            antenna_id = 0
            print 'Making blank entry for ',antenna, antenna_id
            amp_cal_00_all = np.zeros(len(np.median(amps_array[antenna_id,:,:,0],axis=0)))+1.0
            amp_cal_11_all = np.zeros(len(np.median(amps_array[antenna_id,:,:,1],axis=0)))+1.0
            # interpolate phases
            phase_cal_00   = 0.
            phase_cal_11   = np.zeros(len(np.interp(msinfo.msfreqvalues, freqs_phase, phases_array[:,antenna_id])))
            #now handle the clock-value (no fancy interpolating needed)
            clock_pdb = np.zeros(len(np.array( np.median(clock_array[:,antenna_id]) ,ndmin=2)))
...
if __name__ == "__main__":
    if len(sys.argv) < 4 or len(sys.argv) > 5:
        print "usage: %s <msname> <input np dir> <output instrument dir> [npfile_basename = caldata_transfer]"
        sys.exit()
    msname = sys.argv[1]
    npdir = sys.argv[2]
    instdir = sys.argv[3]
